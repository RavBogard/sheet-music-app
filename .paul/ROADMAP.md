...
Status: 🚧 In Progress (3 of 10 phases LOOP COMPLETE — Wave 1 behaviorally complete + Wave 2 closed; HFG counter 0/3; Wave 3 unblocked)
...
| 3 (sequential) | v60-07 | Embedded-array writer removal + immediate `FieldValue.delete` strip | 🚧 In Progress — **v60-07-01 LOOP COMPLETE** 2026-05-13 (mirrorTracksToTopLevel dual-write bridge decommissioned; use-add-to-setlist.ts addToSetlist + addDirectlyToSetlist + undo refactored to engine-path-only via applyEdit Promise.all fanout + denorm-only parent update; +14 LOC net production; 6 existing tests updated to new contract; vitest 1613/52 baseline exact). v60-07 writer-removal template established. **v60-07-02 LOOP COMPLETE** 2026-05-13 (4 create-style writers W1 createSetlist + W3 duplicateSetlist + W4 cloneSetlist + W6 saveAsTemplate refactored to engine-path seeding via shared `seedTopLevelTracks` closure-scoped helper; W5 cloneForNextWeek inherits free; addDoc payloads now carry `trackCount` + `hydrated: true` markers only — no embedded `tracks` field; +61 LOC net production helper-attributable AC-7 DRIFT documented; test contract migrated to mockApplyEdit + `not.toHaveProperty('tracks')`; vitest 1613/52 EXACT baseline match). **v60-07-03 LOOP COMPLETE** 2026-05-13 (W2 updateSetlist defensively strips caller-passed `tracks`; updateSetlistWithVersion emits `tracks: deleteField()` inside the existing transaction when `remote.hydrated === true` — zero extra round-trips; audit-log action collapsed to `'renamed' | 'updated'` with AuditAction union extended; 3 new tests cover defensive strip + hydrated strip + legacy-doc safety; +24 LOC net production across 4 files; AC-6 +2 over ceiling documented as DRIFT; transaction-scoped read-then-strip pattern reusable for future field-cleanup migrations). Remaining: v60-07-04 (API routes W7-W11 — same pattern adapted to Admin SDK). Audit reference: RESEARCH/audit-writes.md. |
| 3 (sequential) | v60-08 | Migration cleanup (delete `mirrorTracksToTopLevel`, drop SSR ternary, drop converter `tracks` field) | ⚪ Not started |
| 4 (parallel after Wave 3) | v60-09 | Cross-device library sync (`library_index ↔ songs/*` continuous) | ⚪ Not started (folded forward from v54-03) |
| 4 (parallel after Wave 3) | v60-10 | Mobile AddBar variant (parallel-render v53-03 split-button + 5-tile popover for mobile breakpoints) | ⚪ Not started (folded forward from v54-?? Mobile AddBar) |
