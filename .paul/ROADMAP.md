...
| v11.6-04 | Authoring + publish round-trip — finalize 3 sets via MCP; airtight publish-&-deliver [P1, live send = STOP-gate] | 01 (WS-11+WS-31) planning · 02 (WS-21 STOP) · 03 (finalize+send STOP) | 🚧 Planning | - |
...
- **03 (autonomous:false — live-send STOP-gate):** Finalize the 3 weekend sets via MCP (keys/BPM/order/bonds airtight → `verify_setlist_charts` clean + `preview_publish` review) then the **LIVE `publish_setlist` send** on Daniel's explicit confirmation (dryRun/preview to test; real-send → `.paul/UAT-PENDING.md`). Closing 03 = v11.6 milestone complete.
