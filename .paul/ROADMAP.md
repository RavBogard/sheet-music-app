...
| 3 (sequential) | v60-06 | Dashboard reader migration + 15-setlist backfill (`migration_snapshots/{setlistId}` rollback collection) | 🚧 In Progress — v60-06-01..06 LOOP COMPLETE 2026-05-12 (trackCount + songCount + fileIds denormalization + 5 reader migrations + bulk Dexie hook + new fetchTracksForSetlistClient Web-SDK helper + TemplatesSection admin migration; +260 LOC net cumulative + 49 hook + 205 test; HFG 0/3 held). Client reader inventory COMPLETE (Dexie-aware 3-branch + Dexie bulk + Web-SDK direct-fetch 2-branch). ~1 reader plan remaining (v60-06-07 matrix/route.ts server-side); v60-06-08 backfill closes phase. |
