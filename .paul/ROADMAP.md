...
| v50-07 | Migration, kitchen-sink, cutover | 3/TBD (01 audit ✓ • 02 MARKER_PATH patch + liveState scrub ✓ • 03 lazy hydration + perf-view dual-read ✓) | 🚧 In progress | - |
...
- **v50-07-04 (PLAN created — awaiting approval)** — Kitchen-sink + minimal lazy-hydration Playwright smoke. Extends `src/lib/sync/__tests__/property-failures.test.ts` with a new `v50-07-04 kitchen-sink` describe that runs fast-check ≥100 iterations on CI (25 local) over a randomized action grammar mixing applyEdit ops + airplane toggles (OfflineToggleAdapter) + force-quits (engine restart with orphaned 'sending' rows) + cross-tab races (TwoWriterAdapter) + lazy-hydration injection (mid-run hydrator mount with hydrated:false), asserting AC-9 no-data-loss + per-doc drain ordering + lazy-hydration idempotency. Reuses setupTwoWriterRace + SharedRemoteSubscriber + OfflineToggleAdapter + FakeClock from v50-06 harness; no copy-paste forks. Optional minimal Playwright spec at `e2e/lazy-hydration.spec.ts` (~200 LOC, gated on Task 0 decision; default `minimal-e2e`) drives the lazy-hydration cascade end-to-end via `page.addInitScript` Dexie pre-seed + thin Firebase shim — no real Firebase project. New `npm run test:kitchensink` script for filtered local re-runs. 1 decision checkpoint (Playwright scope) + 3 auto tasks; 7 ACs (AC-4 decision-dependent); autonomous=false. /ui-ux-pro-max NOT required (test infra, not UI). PLAN at `.paul/phases/v50-07-migration-cutover/v50-07-04-PLAN.md`.
