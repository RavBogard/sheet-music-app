...
| v50-07 | Migration, kitchen-sink, cutover | 4/TBD (01 audit ✓ • 02 MARKER_PATH patch + liveState scrub ✓ • 03 lazy hydration + perf-view dual-read ✓ • 04 kitchen-sink fast-check ✓) | 🚧 In progress | - |
...
- **v50-07-05 (PLAN created — awaiting approval)** — FINAL plan in v50-07 + v5.0 milestone. Three concerns: (1) Sentry alarms on save-path failures via new `src/lib/sync/sentry-capture.ts` helper centralizing tag/level/extra (wraps @sentry/nextjs Sentry.captureException; level='error' for dead-letter/write-atomicity, level='warning' for lazy-hydration/snapshot-listener; try/catches its own SDK calls so telemetry never crashes the engine; tags are string-coerced); wired at 6 silent-failure sites — SetlistGridHydrator lazy-hydration catch (1) + engine.ts dead-letter at line 366 BEFORE the existing dispatch (1) + snapshot-listener.ts setlist-apply / tracks-apply / setlist-subscribe / tracks-subscribe (4). Per-feature explicit non-capture: 'conflict' state (user-facing UX, not failure) + every-drain-attempt (alert fatigue) + payload contents (PII risk on user-authored notes). (2) UAT test plan at `.paul/phases/v50-07-migration-cutover/v50-07-05-UAT-PLAN.md` for Rabbi Daniel + one band member to execute against real production over 1–2 weekly cycles — ≥6 numbered scenarios mapping to weekly-workflow (clone last week + tweak / add new song / bind chart / transpose / mobile flow / open historical legacy setlist exercising lazy-hydration cascade) + 10–15 smoke checklist + coverage map. (3) Ship-to-band checklist at `.paul/phases/v50-07-migration-cutover/v50-07-05-SHIP-CHECKLIST.md` — ≤8 deploy verification steps + 1-page band onboarding doc (open setlist / edit / bind chart / sync indicator semantics in plain English / mobile / what-to-do-if-something-looks-wrong) + first-week Sentry monitoring playbook (alert tag → meaning → severity → response table; rollback via git revert). 3 tasks, 8 ACs, autonomous=true (UAT execution is post-plan; ship is push-to-prod). Boundaries lock engine FSM + adapter interface + write.ts + Dexie schema + SetlistGridHydrator lazy-hydration logic + perf-view + Firestore rules — all instrumentation is purely additive. Scope explicitly excludes: executing UAT, closing v5.0 milestone (separate `/paul:audit-milestone` step post-UAT), new Sentry integrations beyond v1.5 P6 setup, Sentry alerting rules / notification routing (user wires those in dashboard UI from the documented tags), per-applyEdit hot-path captures, public help system migration of the band onboarding doc (drafted in `.paul/`; "Move to public help system in v5.1" noted at the bottom), single-writer offline self-conflict gap from v50-06-03 Block B (deferred-smokes carry forward), legacy `setlists/{id}.tracks[]` cleanup (preserved as backup per v50-07-03). /ui-ux-pro-max NOT required (observability + docs; same precedent as v50-06-01 + v50-07-02 + v50-07-04). PLAN at `.paul/phases/v50-07-migration-cutover/v50-07-05-PLAN.md`.
