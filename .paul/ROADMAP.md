...
**🚧 v11.7 — Discoverability & Deferred Backlog** (OPENED 2026-06-18 · `package.json` → 11.7.0 at close · **7 of 7 phases COMPLETE — ready to close**)
Status: ✅ **All phases complete (7/7)** — awaiting `/paul:complete-milestone` (tag `v11.7.0`). 01 verify-first audit · 02 `find_user` · 03 setlist reverse-lookup · 04 reverse-index sweep · 05 F3 library density · 06 infra org-scoping · 07 authed-broslaz design pass. · **Source:** `.paul/MILESTONE-CONTEXT.md` (consumed at /paul:milestone 2026-06-18). **Oracle:** `docs/ACCESS-POLICY.md` **v0.4** (err-public; cross-tenant hard wall governs the new resolvers).
...
| v11.7-07 | Authed-broslaz design pass + cross-org leader-wall UI check [/ui-ux-pro-max BLOCKING] | 01 `feat(v11.7-07-01)` ✅ (teal-glow bleed fix, BL-scoped/CRC-byte-identical) / 02 `feat(v11.7-07-02)` ✅ (verify-first tap-target + ImporterModal close-out) | ✅ Complete | 2026-06-22 |
...
Plans (✅ COMPLETE 2/2, 2026-06-22):
- **01 `feat(v11.7-07-01)`** — /ui-ux-pro-max audit of the authed BL surface narrowed to ONE P1: the active-state glow was hardcoded CRC indigo (hue 275) on shared nav chrome, so BL's teal theme glowed purple on the desktop active nav link + the mobile tab bar. Fixed BL-scoped: new `:root --nav-active-glow` (= the exact prior CRC literal → CRC byte-identical) + teal hue-210 overrides of `--shadow-brand-glow`/`-soft`/`--nav-active-glow` under `[data-org="brotherslazaroff"]` (MobileTabBar auto-picks teal via `var()`); DesktopHeader inline literal → `shadow-[var(--nav-active-glow)]`. Contrast + nav tap targets verified already-PASS. Leader-wall + teal-glow visual check → UAT-PENDING (non-blocking; data-layer wall shipped v11.7-06). audit: `.paul/research/v11-7-07-01-broslaz-authed-design-audit.md`.
- **02 `feat(v11.7-07-02)`** — verify-first close-out of the folded deferred UI items. The 40→44px sweep was already done at the component level (Button h-11/size-11; Perform v11.6-02; library v11.7-05; nav C10I1-004) → fixed only the genuine band-facing residual: DesktopHeader search Input h-9→h-11. ImporterModal P3 = accept-as-is (low-traffic authoring surface Daniel bypasses via MCP; dense inline-table editors = legit exception). close-out: `.paul/research/v11-7-07-02-tap-target-importer-closeout.md`.
AC PASS both plans; /ui-ux-pro-max held; tsc 0 / nav+org 39/39 / `next build --webpack` 0. SUMMARYs in `.paul/phases/v11.7-07-broslaz-design-pass/`.
...
**✅ v11.6 — Airtight (Weekend Stress & Usability)** (OPENED 2026-06-17 · CLOSED 2026-06-18 · tag `v11.6.0` · `package.json` → 11.6.0 · **4 of 4 phases complete**; archived `.paul/milestones/v11.6.0-ROADMAP.md`)
Status: ✅ **COMPLETE** · Phases: **4 of 4** (01 ✅ discovery · 02 ✅ Perform reading airtight · 03 ✅ off-site resilience · 04 ✅ authoring/publish round-trip) · **Oracle:** `docs/ACCESS-POLICY.md` **v0.4** (access-shaped findings) · **Triage bar widened to usability-AND-access** · **Source:** `.paul/MILESTONE-CONTEXT.md` (consumed at /paul:milestone 2026-06-17). **Discovery report:** `.paul/research/v11-6-01-stress-triage-REPORT-2026-06-17.md` (31 findings WS-01..31; drove 02/03/04). Live publish/notify send intentionally NOT used (Daniel: band opens Perform; no emails) — see [[feedback_no_publish_or_assignments]].
...
| v11.6-03 | Off-site resilience (flaky wifi) — **REFRAMED by Phase-01: offline reading WORKS** (SW+IDB cache re-introduced 2026-05-28) → verify + close narrow residual gaps (WS-10/12/13/29/30), NOT re-introduce. Decision C (full sweep). | 01 verify+decide→C ✅ · 02 WS-12/13 ✅ `7ac307812f` · 03 WS-10 MusicXML IDB-first ✅ `df0bd6b433` · 04 WS-29/30 ACCEPTED residuals ✅ (audio WebKit-blob + soft-nav-offline; visited-URL reload already recovers; no code) | ✅ Complete (4/4) | 2026-06-18 |
| v11.6-04 | Authoring + publish round-trip — finalize 3 sets via MCP; airtight publish-&-deliver [P1, live send = STOP-gate] | 01 ✅ `1a1690478d` (WS-11+WS-31) · 02 ✅ `cdfc16cead` (WS-21) · 03 ✅ finalize (LIVE SEND INTENTIONALLY SKIPPED — Daniel doesn't use publish/notify) | ✅ Complete (3/3) | 2026-06-18 |
...
Plans (set 2026-06-18 — publish/QR round-trip verified already wired vs deployed code; 3 sets already chart-healthy):
- **01 (autonomous):** Deliver-airtight code — **WS-11** (in-app "published" bell links to dead `/setlist/{id}`; push/SMS correct → point both in-app sites to `/perform/setlist/{id}`) + **WS-31** (QR phone page treats not-yet-registered 404 as "expired" → bounded retry distinguishing 404 from 410). Regression + tsc + `next build --webpack`. /ui-ux-pro-max before the QR-page task.
- **02 (autonomous:false — auth STOP-gate):** **WS-21** — QR approval hard-requires a `role` claim; a claim-lagged but approved musician gets 403. Remedy = user-doc role fallback / in-flow recovery. Permission-surface change → approval before APPLY.
- **03 (autonomous:false — live-send STOP-gate) ✅ DONE 2026-06-18:** Finalized the 3 weekend sets via MCP — corrected the stale staff-concert ID (live = `c2075393`, NOT the "(OLD)" `7c640a8a`), all 3 verified chart-clean, 2 safe key fills applied (Eliyahu-fast→Dm, Hallelujah→G). **LIVE SEND INTENTIONALLY NOT PERFORMED** — at the STOP-gate Daniel said he doesn't use the publish/notify feature (band opens Perform directly; no emails wanted) and uses scheduling only for gig dates (no musician assignments). The send-gate did its job. See [[feedback_no_publish_or_assignments]]. Closing 03 = v11.6 milestone complete.
...
## Earlier Completed Milestone — v11.4 Publish & Notify (D8) (✅ COMPLETE)
