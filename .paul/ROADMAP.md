...
- **v11-02-03** 🚧 PLAN created (2026-06-08) — Org-scope MCP WRITES: stamp caller org on every MCP create path; deny cross-tenant mutation of existing docs (resolve target doc's orgId, return the standard **not-found** envelope when it ≠ caller org — refined from the earlier `cross_tenant_denied` sketch to avoid leaking cross-tenant existence, mirroring v11-02-02 #3). Chokepoint = `loadEditableSetlist` (8 setlist tools) + per-tool guards (delete/clone/recompute/update_song/delete_chart) + create-stamp on create/clone/template/3 chart-create tools (via `stampOrg`). Emulator isolation test. Autonomous, 3 tasks.
- **v11-02-04** 🚧 PLAN created (2026-06-08, LAST in v11-02) — Issue David's `brotherslazaroff` bearer (mint via tokens.ts; `orgId` on the mcpTokens doc) + set his `orgIds:['brotherslazaroff']` claim by MERGE (preserve role:band_leader) on his EXISTING band_leader account + onboarding doc; **ship feat(v11-02) to prod** (pull-first → commit → push → Vercel deploy, folded into APPLY since the live e2e needs deployed code) → live end-to-end verification against deployed /api/mcp (David lists only BL, create stamps BL, CRC-id read/mutate denied, CRC bearer unaffected). Executor = this box (Daniel-authorized). Autonomous, 3 tasks.
