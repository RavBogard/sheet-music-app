...
|------|-------|-------|--------|
| 0 (foundation) | v70-01 | **Image-chart support** (PNG / JPEG / HEIC) — upload route ALLOWED_TYPES extended; chart viewer branches PDF→pdfjs vs image→`<img>`; print pipeline embeds images; AI chord detection + transposition DISABLED for image charts with explanation tooltip | 🟠 Paused mid-APPLY 2026-05-13 (Tasks 1+2+4 shipped @ ab11850; Task 3 deferred — pivoted to v60-13 P0 hotfix cluster) |
| EMERGENT (interrupt) | v60-13 | **Sync-engine resilience hotfix** (post-v6.0-close emergent) — Daniel UAT 2026-05-13 surfaced 3 P0/P1 issues clustering on sync-engine + auth-gate fragility: (1) incognito users see no setlists (use-setlist-dashboard.ts:87 over-restrictive gate); (2) mobile dashboard blanks after flash; (3) "failed/conflict" toasts noisy during edits. v60-13-01 = drop auth gate (single-file fix). v60-13-02 = VersionMismatch tuning (TBD). | 🚧 In progress (v60-13-01 plan in flight) |
