...
|-------|------|-------|--------|-----------|
| v5h-01 | Track-edit save-loss diagnosis + fix | 4 (planned: 01-01 reproduce+diagnose ✓ • 01-02 fix ✓ apply • 01-03 perf-view bridge fix • 01-04 postmortem) | 🚧 In progress | - |

### Phase v5h-01: Track-edit save-loss diagnosis + fix

Focus: Reproduce Daniel's flow in a kitchen-sink scenario (fresh setlist, no legacy tracks, edit key, simulate page-nav with cached pre-edit Firestore delivery via snapshot listener), capture production state with DevTools open (HUMAN-ACTION), pick fix shape from three candidates (A: writeback never fired → unconditional + verified; B: listener LWW underflow → guard against undefined local.updatedAt; C: serverTimestamp didn't resolve → switch to client-side Date.now()), ship fix with the regression test from 01-01 locking it, then close perf-view bridge that surfaced as a separate failure mode in Daniel's UAT, then postmortem the harness-fidelity gap.

Plans:
- **v5h-01-01 — Reproduce + diagnose** ✅ COMPLETE 2026-04-27. Production capture revealed the actual root cause was missing Firestore rules for top-level `tracks/{id}` and `songs/{id}` collections (default-deny rejected every v5.0 track save). SUMMARY at `.paul/phases/v5h-01-track-edit-save-loss/v5h-01-01-SUMMARY.md`. Decision = E + F + B defense-in-depth.
- **v5h-01-02 — Fix (E + F + B)** ✅ APPLY COMPLETE 2026-04-27 (commit 0c2921d, suite 1479/1479, rules deployed). E = firestore.rules tracks+songs blocks; F = Hydrator outbox-pending guard; B = snapshot-listener strict-equality LWW guard + AC-1 regression lock. UNIFY blocked on AC-4 (Daniel UAT) — partial pass: editor save works, perf-view doesn't reflect → routes to 01-03.
- **v5h-01-03 — Perf-view bridge fix** (execute; ~1-2h). Surfaced in v5h-01-02 HUMAN-VERIFY: Daniel's edit landed in `tracks/{id}` (editor shows it) but perf-view still showed the stale legacy `setlists/{id}.tracks[]` embedded array. Root causes: (a) `useSetlistPerformance` `onSnapshot` error handler sets `topLevelTracks=[]` and never resubscribes — if the perf-view tab opened before rules deployed, the subscription is permanently dead; (b) the dual-read `topLevelTracks.length > 0 ? topLevelTracks : setlistData?.tracks` falls through to the legacy embedded array even for hydrated setlists, where embedded is intentionally stale post-migration. Fix: `useSetlistPerformance` always trusts top-level when `setlistData?.hydrated === true`; on `onSnapshot` error, retry with backoff (capped). +3 tests (prefer-top-level-when-hydrated, fallback-when-not-hydrated, resubscribe-on-error). After ship → push to prod → Daniel re-runs UAT scenario 1 with perf-view check + Issue 2 (iPad key-picker UI) reproduction for v5.1 routing.
- **v5h-01-04 — Postmortem** (execute; ~30min) — `.paul/postmortems/v50-07-save-loss.md`: what kitchen-sink missed (in-memory adapters with zero latency missed the snapshot-listener cache-vs-fresh delivery race AND the cutover-introduced collections-without-rules gap); cutover-plan rules-audit gate proposal; production capture should precede harness work in future v5h plans.
