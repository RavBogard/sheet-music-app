...
Status: 🚧 In Progress — 1 of 8 phases complete. v70-01 (image-chart support) ✅ COMPLETE 2026-05-14 (2 plans / both LOOP CLOSED). Next: v70-02 recordings data model (Wave 1 foundation).
...
| 1 (foundation) | v70-02 | Recordings data model + Firestore rules + Storage paths — NEW `recordings/{id}` collection; foreign key `songId?`; `notes` field for attribution; Firebase Storage path; HFG-relevant (emulator coverage required) | 🟡 Planning — v70-02-01 PLAN created 2026-05-14 (Recording type + getRecordingStoragePath + rules block + composite index + emulator rules test; 3 auto tasks, autonomous) |
