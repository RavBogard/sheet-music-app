...
| v50-05 | Spreadsheet editor UI (cutover) | 2/5 (01 build ✓ • 02 cutover ✓ • 03 multi-select+AlertDialog • 04 iPad+ContextMenu • 05 mobile+WCAG+undo) | 🚧 In Progress | - |
...
- **v50-05-03 — Multi-select / batch edit + AlertDialog swap-in.** §6.6 multi-select via Cmd/Shift-click on the drag handle + sticky BatchActionBar (Type / Key / Lead / Delete; bulk applyEdit + per-songId propagation; selection preserved across bulk-set, cleared on bulk-delete). AlertDialog (shadcn) replaces window.confirm via a `<DeleteConfirmProvider>` context wrapper at /setlists/[id]; SetlistGrid picks up `confirmDelete` from context with prop-injection fallback for tests. 3 tasks; ~12+ new vitest cases. /ui-ux-pro-max BLOCKING for APPLY.
- **v50-05-04 — iPad / pointer-coarse touch variant + right-click ContextMenu.** §6.7 — Cell dropdowns (Key, Lead, Type, Add-Row, Chart-bind) swap from Radix Popover → Radix Sheet (bottom sheet) when `useMediaQuery('(pointer: coarse)')` matches (NOT viewport width — iPad Pro at 1024px is still touch). 44px minimum touch targets (cell padding bumped from 8px → 12px on touch breakpoints). Drag-handle column wider (44px → 52px) for tap accuracy. Hover-only affordances (drag-handle reveal, chart icon ghost) become always-visible OR get long-press equivalents on touch. Right-click ContextMenu (Radix ContextMenu) on rows + drag handle: "Edit row" (focuses Title cell), "Bind chart" (opens ChartBindPopover programmatically), "Duplicate row", "Delete row". Plan size: ~3 tasks. /ui-ux-pro-max BLOCKING for APPLY.
- **v50-05-05 — Mobile stacked-card flow + WCAG AA audit + Undo via zustand temporal middleware.** §6.11 — Below 768px (and only there; the existing table layout breaks ~640px so this is a parallel render path, not a Tailwind responsive trick), drop the table entirely and render rows as stacked cards with primary fields visible (title + key + lead). Tap card → full-screen Sheet with all-fields edit pane. Reorder via long-press + drag (or up/down buttons in the sheet). §6.13 — Run axe-core / Lighthouse against /setlists/[id] on prod; fix any focus-trap gaps in popovers (verify cmdk inside Popover.Content); verify keyboard-only navigation across all cells + add-row + chart-bind + delete; verify aria-live announcement timing for SyncIndicator state changes; verify color contrast ratio ≥ 4.5:1 for all SyncIndicator states. Undo via zustand temporal middleware: wrap a small zustand store around local Dexie writes; intercept BEFORE applyEdit; record (op, collection, docId, prevDoc, newDoc) snapshot per undo unit; Cmd/Ctrl-Z replays the inverse via applyEdit (NOT direct db.put — inverse should round-trip to Firestore); coalesce burst edits per cell-blur; cap 50 undo entries; not persisted across reloads (Dexie is the persistence layer). Plan size: ~3 tasks. /ui-ux-pro-max BLOCKING for APPLY.
...
*Last updated: 2026-04-26 (v50-05-02 closed: cutover landed — SetlistGrid mounted via SetlistGridHydrator at /setlists/[id]; ChartBindPopover wires chart binding; ~−6,300 LOC legacy editor surface deleted; v50-05-03 polish next)*
