...
| 3 (sequential) | v60-07 | Embedded-array writer removal + immediate `FieldValue.delete` strip | 🚧 In Progress — **v60-07-01 LOOP COMPLETE** 2026-05-13 (mirrorTracksToTopLevel dual-write bridge decommissioned; use-add-to-setlist.ts addToSetlist + addDirectlyToSetlist + undo refactored to engine-path-only via applyEdit Promise.all fanout + denorm-only parent update; +14 LOC net production; 6 existing tests updated to new contract; vitest 1613/52 baseline exact). v60-07 writer-removal template established. Remaining: v60-07-02 (setlist-firebase.ts W1-W6) + v60-07-03 (API routes W7-W11). Audit reference: RESEARCH/audit-writes.md. |
