...
- [ ] **Supply the three page numbers deliberately left blank on both RH services.** Each names something the machzor prints, but the only indexed match sits at a page that would be wrong, so the resolver refused it rather than guess. (a) **Chatzi Kaddish** — placed after Yishtabach p.103, just before Bar'chu; the only morning Chatzi Kaddish is `awakening.chatzi-kaddish@shabbat-shacharit` p.62, a different instance 41 pages back. (b) **The Great Aleinu** — only `concluding.aleinu@rh-shacharit` p.200 is indexed; the Great Aleinu in the shofar service is a distinct moment. (c) **Blessing over the Shofar & Shehecheyanu** — matched the closing `concluding.shehecheyanu@rh-shacharit` p.205, but the row sits near Malchuyot ~p.183. Give the three folios and they can be set on both services in one `bulk_update_tracks` pass.
