...
- [ ] **Read a real outline on a real iPad, standing.** Open a `/perform/setlist/<id>` with page numbers on an 11" iPad at music-stand distance. Check: the page number is findable at a glance without hunting; the folios line up as a column down the list across song, prayer, header, and bonded-chart rows; the honoree line doesn't crowd the title; a two-line description doesn't make rows feel bloated. Type sizes were raised from 14px to 16px on outline rows for exactly this — confirm it reads.
- [ ] **Print a gig packet for a 30+ track service.** The cover table now paginates; a 34-track set draws 30 rows then 4 on page 2, headers repeating, title/date on page 1 only. Confirm on paper that nothing is lost at the break and the second page looks deliberate rather than orphaned. *(Before this fix, a 30-track service silently printed only ~26 rows — Aleinu and Mourner's Kaddish simply vanished.)*
- [ ] **DECISION: should a track's book be forced to match the setlist's book?** Nothing currently couples `track.liturgyRef.book` to `Setlist.book` — they validate independently. The Perform header names the *setlist's* book while each row's number comes from the *track's*. Divergent authoring would put a correct page number under a wrong book name, and the Friday/Saturday siddurim share 132 prayer names at different pages. Options: enforce on the MCP write path (rejects deliberate cross-book rows), warn only, or leave. Deferred deliberately rather than guessed at.
