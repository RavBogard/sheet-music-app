...
| AC-3: Backfill script seeds 134 historical missing songs/* docs | ✅ PASS | Production --dry-run: 498 lib / 364 songs / 131 missing / 3 skipped empty-name (134 total gap as predicted). Production --apply (2026-05-13, Daniel-authorized): 131 docs written. Songs total 364 → 495. Marker `system/v60-11-backfill` set. Spot-check via diag-lechu-goldman.ts: songs/1jgs72zw...VJj NOW exists with title "Lechu Goldman.pdf" (verbatim per A1 audit fix). 364 existing docs untouched (sticky memory preserved by append-only contract). Idempotent on re-run (verified by marker logic). |
| AC-4: Production UAT — "Lechu Goldman" appears in picker | ⏳ PENDING | Carry-forward per v51-04 codified pattern. Backfill applied + deployed; Daniel verifies picker on iPad post-Vercel-deploy: type "Lechu" → "Lechu Goldman.pdf" appears within ~1s via v60-09 continuous listener + select binds chart + /perform/{id} renders resolved PDF. Verify 2+ other previously-missing shortcuts surface. Worship cycle Fri PM + Sat AM. |
