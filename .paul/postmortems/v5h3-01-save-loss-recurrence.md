...
| 2 | Open v5.4 milestone with first phase scoped to harness-fidelity remediation: Firebase emulator integration in property-failures harness (covers Gap A + C) + thin RTL editor↔perf-view cross-view propagation test (covers Gap B). | Rabbi Daniel | At v5.3 milestone close (post-band-onboarding UAT cycle) | **GATES** v5.4+ data-flow phases per Action #1. **✅ CLOSED 2026-05-12 by v60-03-01.** Emulator infra shipped in v54-02-01 (firebase.json + vitest.emulator.config.ts + CI emulator-tests job). H-SL-7 regression canary shipped in v60-03-01 (`src/lib/sync/__tests__/engine.emulator.test.ts` — full-stack engine + emulator + Dexie test of the rapid-same-doc-edit scenario). Counter reset to 0/3 backed by *proof* per locked decision Q6: local revert of the engine.ts:282-321 writeback block makes the canary fail (engine never drains; 10s polling deadline expires); restoring it brings the canary back to GREEN in ~350ms. RTL editor↔perf-view cross-view propagation pair NOT shipped here — Gap B remains tracked for future Wave 3+ work if a propagation-class bug surfaces; counter mechanism stays in place to catch it. v53-02 clause-(b) waiver: RESOLVED. |
