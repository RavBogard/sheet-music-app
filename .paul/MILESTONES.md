...
| v11.7 Discoverability & Deferred Backlog | 2026-06-22 | ~4 days (2026-06-18 → 2026-06-22) | 7 phases, 11 plans; MCP **Asymmetry Principle** — every act-by-id write tool gained a discovery/reverse-read path (01 verify-first audit · 02 `find_user` [the David-is-invisible fix; instrument gate dropped] · 03 `find_setlists_referencing_chart` + `search_setlists` · 04 `find_setlists_from_template` + `find_contact`, all index-free + tenant-walled) PLUS folded deferred backlog (05 F3 library density text-only · 06 infra cross-tenant org-scoping: recordings + finalize signed-URL + anon chord-cache · 07 authed-broslaz design pass: CRC-indigo→teal glow bleed fix BL-scoped + tap-target/ImporterModal verify-first close-out). err-public within tenant / hard wall across; CRC byte-identical where shared; P4/P5 `list_recent_*` deferred → v11.8 (infra-blocked). Oracle v0.4; tag `v11.7.0` |
