...
```
PLAN ──▶ APPLY ──▶ UNIFY
  ✓        ○        ○     [v5h-01-02 PLAN created, awaiting approval → /paul:apply]
...
v5h-01-02:     ✓ ──▶ ○ ──▶ ○     [PLAN created at .paul/phases/v5h-01-track-edit-save-loss/v5h-01-02-PLAN.md; type=execute; autonomous=false; depends_on=["01"]; 1 decision-checkpoint at start (option-all-three vs option-ef-only) + 5 auto tasks + 1 HUMAN-VERIFY at end (Daniel UAT scenario 1). Tasks: T1 firestore.rules add match /tracks/{trackId} + match /songs/{songId} blocks mirroring setlists pattern (band_leader/admin write, member read; drop ownerId check) + deploy via `firebase deploy --only firestore:rules` (Firebase CLI available); T2 SetlistGridHydrator.tsx hydrate() outbox-pending guard for both setlists + tracks puts (mirrors snapshot-listener.ts:197); T3 SetlistGridHydrator.test.tsx +3 tests proving the guard; T4 (decision-gated all-three) snapshot-listener.ts:174 + :215 strict-equality LWW guard against undefined local.updatedAt; T5 (decision-gated all-three) flip AC-1 from it.fails → it in property-failures.test.ts as regression lock. Recovery snippet for Daniel's 50+ stuck outbox rows documented inline. Boundaries lock engine.ts FSM + adapter interface + init.ts + write.ts + Dexie schema + sentry-capture + cross-tab-lock + state-machine + lazy-hydration cascade effect (lines 134-191) + all firestore.rules blocks outside new tracks+songs.]
v5h-01-03:     ○ ──▶ ○ ──▶ ○     [Postmortem; planned after 02 ships — covers kitchen-sink security-rules fidelity gap, cutover-plan rules-audit gate, deferred concerns]
v5h-01-02:     ○ ──▶ ○ ──▶ ○     [Fix; ships regression test from 01-01]
v5h-01-03:     ○ ──▶ ○ ──▶ ○     [Postmortem]
