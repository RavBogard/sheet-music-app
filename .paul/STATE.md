...
Plan: **v11-04-02 ✅ LOOP COMPLETE + SHIPPED + LIVE-VERIFIED** (`feat(v11-04-02)` `f50060e387` pushed origin master, Vercel deploy live ~04:38; prod probe confirms BL /perform tab title "Upcoming Shows & Sets | Brothers Lazaroff" + "Brothers Lazaroff" wordmark, CRC byte-identical). → next: **v11-04-03** (NOT STARTED — ready to plan): David onboarding + empty-library verify + **authed-dashboard read scoping** (getUpcoming/getRecent/getSetlistsPage + non-/perform subscribeToAllSetlists callers: DashboardClient, SetlistDrawer, use-add-to-setlist, use-setlist-dashboard) + e2e UAT. Decomposition (v11-04): **01 ✅** web-read scoping · **02 ✅** org-aware consumer branding/metadata · **03 ⏳** onboarding + authed-read scoping + UAT. **DEFERRED to v11-05:** templates/roster/congregation/service-personnel R+W + CreationWizard vocab.
...
```
PLAN ──▶ APPLY ──▶ UNIFY        [v11-04-02 LOOP COMPLETE — next: PLAN v11-04-03]
  ✓        ✓        ✓     (v11-04-01 + v11-04-02 both SHIPPED+LIVE-VERIFIED)
```
...
Next action: **/paul:plan v11-04-03** — David onboarding + empty-library verify + **authed-dashboard read scoping** (the still-cross-tenant `getUpcomingSetlists`/`getRecentSetlists`/`getSetlistsPage` via `server-auth.ts` + `(main)/setlists` + `/api/setlists/page`, AND the non-/perform `subscribeToAllSetlists` callers: DashboardClient, SetlistDrawer, use-add-to-setlist, use-setlist-dashboard) + e2e UAT. The opt-in `org` filter exists on `getAllSetlists`/`subscribeToAllSetlists` (v11-04-01) — 03 wires the authed callers to pass org. Then phase v11-04 closes. **v11-05** = cross-tenant collection scoping (templates/roster/congregation/personnel R+W + CreationWizard vocab); **v11-06** = isolation audit.
