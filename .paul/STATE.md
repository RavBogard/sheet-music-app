...
Plan: v50-07-02 LOOP COMPLETE — Option C Hybrid selected by user. MARKER_PATH bug patched in `migrate-v50.ts` (was 3-seg collection 'system/migrations/v50', now 2-seg doc 'system/v50Migration'); 13 existing migrate-v50 tests still pass. New `scripts/scrub-livestate.ts` (~250 LOC, dry-run by default) + 14 unit tests; reuses MigrationFirestore + FIELD_DELETE_SENTINEL. Production scrub applied: 10 setlists' `liveState` field removed via FieldValue.delete with per-setlist snapshots at `migrations/livestate-scrub/snapshot/{setlistId}` for rollback; marker at `system/livestateScrub`. Re-audit confirms liveState count = 0; setlists/tracks counts unchanged. Suite 1456/1456 (+14); tsc + next build clean. SUMMARY at `.paul/phases/v50-07-migration-cutover/v50-07-02-SUMMARY.md`.
Last activity: 2026-04-27 — v50-07-02 closed; v50-07-03 (lazy hydration + perf-view dual-read; /ui-ux-pro-max BLOCKING) is next.
