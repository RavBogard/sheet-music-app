...
v50-07-04..05: ○ ──▶ ○ ──▶ ○     [v50-07-04 Playwright kitchen-sink (next); v50-07-05 Sentry alarms + UAT prep + ship-to-band]
```

## How to resume

Run `/paul:plan` for v50-07-04 — Playwright kitchen-sink (≥100 runs random edits + airplane-mode toggles + force-quits + cross-tab edits = zero data loss). Reuse `setupTwoWriterRace` + `SharedRemoteSubscriber` + `OfflineToggleAdapter` patterns from the v50-06 property-failures harness; now also exercise the v50-07-03 lazy-hydration cascade (open a legacy setlist mid-fan-out and assert per-doc drain ordering still holds under failure injection). `/ui-ux-pro-max` NOT required for v50-07-04 (test infra, not UI). After v50-07-04 closes: v50-07-05 (Sentry alarms on save-path failures + manual UAT with Rabbi Daniel + ship-to-band) → milestone audit (`/paul:audit-milestone` if available, else `/paul:plan-milestone-gaps`) → v5.0 milestone close.
