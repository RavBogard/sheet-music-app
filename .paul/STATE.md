...
Phase: **v11.5-01 of 5 — Tenancy + anon correctness (H4 + H5 + H9)** [P0/P1] — **In progress (1 of 3 plans complete).** **01 = H4** (Perform-setlist header branding leak, UI) ✅ DONE · **02 = H5** (anon chord-cache write 401, server) · **03 = H9** (band_leader library-edit lockout — field-reported 2026-06-12 by David; STOP-gate MCP permission change). H9 added 2026-06-12: relax `editEnrichment`'s `assertAdmin` gate → admin-OR-band_leader for tags/title/key/bpm/leadMusician (collection stays admin) + add org-scoping (`row.org ∈ caller orgIds`); immediate unblock = temp-bump David to admin.
