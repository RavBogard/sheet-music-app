...
**Current focus:** v5.0-hotfix — diagnose + fix the P0 track-edit save-loss surfaced by Daniel's UAT (path "P" confirmed). Blocks v5.0 milestone close.

## Current Position

Milestone: 🚧 v5.0-hotfix — Track-Edit Save-Loss Fix — 0 of 1 phases complete. P0 surfaced by Daniel's UAT 2026-04-27. Per archived handoff `.paul/handoffs/archive/HANDOFF-2026-04-27-post-uat-v5h-and-v51.md`.
Phase: v5h-01 of 1 (Track-edit save-loss diagnosis + fix) — 🚧 Planning; 3 plans planned (01-01 reproduce+diagnose ✓ created • 01-02 fix • 01-03 postmortem).
Plan: v5h-01-03 PLAN CREATED 2026-04-27 — awaiting approval. Located at `.paul/phases/v5h-01-track-edit-save-loss/v5h-01-03-PLAN.md`. Type=execute, autonomous=false (1 HUMAN-VERIFY at end = Daniel UAT scenario 1 with explicit perf-view check). 2 auto tasks: (T1) `src/hooks/use-setlist-performance.ts` hydrated-trust dual-read + onSnapshot resubscribe-once with 1s delay; (T2) +3 tests in `src/hooks/__tests__/use-setlist-performance.test.ts` (prefer-top-level-when-hydrated, fallback-when-not-hydrated, resubscribe-on-error). Closes the perf-view side of v5.0 save-loss bug surfaced by Daniel's v5h-01-02 UAT (editor save works post-E+F+B; perf-view didn't reflect because (a) onSnapshot subscription dies on permission-denied with no resubscribe and (b) dual-read falls back to stale legacy embedded array even for hydrated setlists). /ui-ux-pro-max NOT required — data-layer hook only. After ship: push to prod → Daniel re-runs UAT scenario 1 with perf-view check → if PASS, run `/paul:unify .paul/phases/v5h-01-track-edit-save-loss/v5h-01-02-PLAN.md` AND `/paul:unify .paul/phases/v5h-01-track-edit-save-loss/v5h-01-03-PLAN.md`, then plan v5h-01-04 postmortem.

Earlier: v5h-01-02 APPLY 5/5 auto tasks COMPLETE 2026-04-27 (commit 0c2921d; suite 1479/1479; tsc + build clean; Firebase rules deployed). UAT 2026-04-27: editor save works (E+F+B landed), perf-view didn't reflect the edit. AC-4 PARTIAL → routed to v5h-01-03 (perf-view bridge fix). Issue 2 (iPad key-picker UI bad) routed to v5.1 OR v50-05-04 regression follow-up depending on Daniel's symptom description. UNIFY for v5h-01-02 still blocked on AC-4 — will resolve after v5h-01-03 ships + Daniel's combined UAT confirms.

Earlier: v5h-01-01 LOOP COMPLETE 2026-04-27 — reproduction harness shipped + production capture revealed actual root cause = missing Firestore rules for tracks/{id} + songs/{id}; decision = E+F+B defense-in-depth. SUMMARY at .paul/phases/v5h-01-track-edit-save-loss/v5h-01-01-SUMMARY.md.

Earlier: v5h-01-01 PLAN was created — original (now-superseded) details: Located at `.paul/phases/v5h-01-track-edit-save-loss/v5h-01-01-PLAN.md`. Type=research, autonomous=false (1 HUMAN-ACTION + 1 decision-checkpoint). 3 tasks: (1) CachedThenFreshSubscriber + reproduction harness extending property-failures.test.ts with AC-1 expected-failure test + AC-2 counter-test scoping the bug to engine-undefined-writeback ∩ listener-undefined-underflow conjunction; (2) HUMAN-ACTION Daniel captures production Dexie state with DevTools open BEFORE clearing browser data → `.paul/postmortems/v50-07-save-loss-investigation.md`; (3) decision-checkpoint A/B/C/D (A: engine writeback unconditional + adapter contract tightening; B: listener LWW guard against undefined local.updatedAt — 2-line surgical fix; C: production adapter Date.now() instead of serverTimestamp — large blast radius; D: B+A combination defense-in-depth, recommended pre-evidence). Boundaries lock all production sources (snapshot-listener.ts + engine.ts + init.ts + firestore-adapter.ts + sentry-capture.ts) — research only. Smoking-gun observation captured in plan context: snapshot-listener.ts:215 `(local.updatedAt ?? 0) >= change.updatedAt` underflows when local has undefined updatedAt; engine.ts:264 gates writeback on `result.updatedAt !== undefined` — conjunction = clobber on cached delivery. /ui-ux-pro-max NOT required (engine + harness; same precedent as v50-06-01 + v50-07-04). Output v5h-01-01-SUMMARY.md will record decision + specific files/lines for v5h-01-02 to consume. Reproduce Daniel's exact flow in property-failures.test.ts kitchen-sink (fresh setlist + applyEdit set track no key + drain + applyEdit update key=E + drain + simulate page-nav by clearing Dexie tracks + re-priming via Hydrator initialTracks=[] + mocked snapshot delivery using PRE-edit cached Firestore state); HUMAN-ACTION checkpoint for Daniel to capture production Dexie state with DevTools open (Application → IndexedDB → crc-sync → tracks store: confirm key='E' + updatedAt is a number not undefined BEFORE navigating, then again after); root-cause confirmation + fix-shape decision A/B/C. **CRITICAL:** Daniel must NOT clear browser data on the affected setlist before this capture.
...
Last activity: 2026-04-27 — v50-07-03 LOOP COMPLETE; v50-07-04 (Playwright kitchen-sink) is next.
...
```
PLAN ──▶ APPLY ──▶ UNIFY
  ✓        ○        ○     [v5h-01-03 PLAN created 2026-04-27, awaiting approval → /paul:apply. v5h-01-02 partial UAT pass: editor save WORKS, perf-view doesn't reflect → 03 closes the perf-view bridge.]

v5h-01-01:     ✓ ──▶ ✓ ──▶ ✓     [LOOP COMPLETE 2026-04-27. Reproduction harness shipped + production capture revealed actual root cause = missing Firestore rules for tracks/{id} + songs/{id}; decision = E+F+B defense-in-depth. SUMMARY at .paul/phases/v5h-01-track-edit-save-loss/v5h-01-01-SUMMARY.md.]
v5h-01-02:     ✓ ──▶ ◐ ──▶ ○     [APPLY 2026-04-27: ship E+F+B (option-all-three). Suite 1479/1479; tsc+build clean; commit 0c2921d pushed; rules deployed to crcmusiccharts. SUMMARY at .paul/phases/v5h-01-track-edit-save-loss/v5h-01-02-SUMMARY.md. UAT 2026-04-27: editor save WORKS (E+F+B fix landed), perf-view DID NOT reflect the edit. AC-4 PARTIAL → routed to v5h-01-03 (perf-view bridge fix).]
v5h-01-03:     ✓ ──▶ ○ ──▶ ○     [PLAN created 2026-04-27 at .paul/phases/v5h-01-track-edit-save-loss/v5h-01-03-PLAN.md. Type=execute; autonomous=false; depends_on=["02"]; 2 auto tasks + 1 HUMAN-VERIFY. Tasks: T1 useSetlistPerformance hydrated-trust dual-read (when setlistData.hydrated===true, ALWAYS use topLevelTracks; never fall back to stale legacy embedded array) + resubscribe-once on onSnapshot error (1s delay; cleanup-cancellable; budget=1 retry, no storm); T2 +3 tests (prefer-top-level-when-hydrated, fallback-when-not-hydrated, resubscribe-on-error). HUMAN-VERIFY = Daniel re-runs UAT scenario 1 with explicit perf-view check. Boundaries lock engine.ts + snapshot-listener.ts + Hydrator + init.ts + adapter contract + firestore.rules + perf-view rendering components. /ui-ux-pro-max NOT required (data-layer hook only). Files modified: src/hooks/use-setlist-performance.ts + src/hooks/__tests__/use-setlist-performance.test.ts. Issue 2 (iPad key-picker UI bad) explicitly OUT OF SCOPE — routed to v5.1 OR v50-05-04 regression follow-up depending on Daniel's symptom description.]
v5h-01-04:     ○ ──▶ ○ ──▶ ○     [Postmortem; planned after 03 closes — covers kitchen-sink security-rules + perf-view-coverage fidelity gap, cutover-plan rules-audit gate, perf-view integration test recommendations, UI/UX feedback loop with Daniel during v5.x UAT cycles.]
...
**v5h-01-02 APPLY 5/5 auto tasks COMPLETE; HUMAN-VERIFY DEFERRED.** Code is shipped to production (commit 0c2921d, Vercel auto-deploy in flight; Firebase rules already live). User said "I'll verify l8r" — once Daniel runs UAT scenario 1 against prod and reports back, run `/paul:unify .paul/phases/v5h-01-track-edit-save-loss/v5h-01-02-PLAN.md` to close the loop. UAT recovery snippet for Daniel's stuck outbox is documented in v5h-01-02-SUMMARY.md + the HUMAN-VERIFY checkpoint in the PLAN.

If user resumes BEFORE Daniel's UAT lands: surface the deferred HUMAN-VERIFY as the single next action (UAT cannot be skipped — AC-4 closes the bug claim end-to-end). v5h-01-03 postmortem can be DISCUSSED in parallel via `/paul:discuss` but should not be planned until UAT confirms the fix actually lands.

If UAT FAILS: diagnose specific step (rules deploy propagation, sync indicator state, tracks store, Sentry events tagged feature:dead-letter or feature:write-atomicity) and route to either Task amendment or new follow-up plan v5h-01-03b.
