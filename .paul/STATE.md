...
Plan: v50-07-01 LOOP COMPLETE (audit shipped); HUMAN-VERIFY pending for v50-07-02 scope decision. SUMMARY at `.paul/phases/v50-07-migration-cutover/v50-07-01-SUMMARY.md`. DRY-RUN-REPORT at same path. Audit found: 29 setlists (24 with embedded tracks, 650 total tracks, 0 distinct songIds — legacy shape uses `id`/`fileId` not `songId`); songs/* empty; tracks/* empty (v5.0 editor unused in prod); 10 setlists with `liveState` orphan (chats/songGroups already clean). 🐛 Pre-existing bug in `migrate-v50.ts`: MARKER_PATH 'system/migrations/v50' is 3-segment = collection, not doc; tests use fake adapter so never surfaced; would crash on first apply. Recommendation: Option C Hybrid (Lazy Hydration in SetlistGridHydrator + scrub liveState + patch MARKER_PATH). Awaiting user choice between Option A (Full Forward Migration), B (Clean-Slate Cutover), C (Hybrid — recommended), override, or pause.
...
v50-07-01:     ✓ ──▶ ✓ ──▶ ✓     [Audit shipped; HUMAN-VERIFY pending for v50-07-02 scope]
v50-07-02..05: ○ ──▶ ○ ──▶ ○     [Blocked on HUMAN-VERIFY — user picks Option A/B/C; recommended C (Hybrid Lazy Hydration)]
```
