...
Plan: **v11-04-03 PLAN CREATED (awaiting APPLY — /ui-ux-pro-max BLOCKING)** — org-scope the remaining AUTHED setlist reads so a signed-in BL user's /setlists dashboard shows BL-only (currently shows CRC). Task 1: opt-in `org` on `getSetlistsPage` (+getUpcoming/getRecent parity), mirroring getAllSetlists' `.where('orgId','==',org)` + deployed (orgId,date) index. Task 2: thread org from `x-org-id` into both getSetlistsPage callers ((main)/setlists/page.tsx SSR + /api/setlists/page route). Task 3: pass `useOrg()` to the 4 non-/perform subscribeToAllSetlists callers (DashboardClient, SetlistDrawer, use-add-to-setlist, use-setlist-dashboard). Plan at `.paul/phases/v11-04-consumer-surface-onboarding/v11-04-03-PLAN.md`. **DEFERRED/FLAG:** in-app setlist CREATE orgId stamping → v11-05 write-scoping. Decomposition (v11-04): **01 ✅** web-read scoping · **02 ✅** branding/metadata · **03 ⏳ PLAN created**. **DEFERRED to v11-05:** templates/roster/congregation/service-personnel R+W + CreationWizard vocab.
Status: **v11-04-02 COMPLETE + committed + pushed + LIVE-VERIFIED.** Consumer-surface branding is now data-driven (branding.ts metadata fields, CRC byte-identical) — root `generateMetadata()` + per-org manifest + org-aware /perform wordmark/aria-label + de-synagogued listing title (vocab `publicListingTitle`). Last CRC-remnant signals on brotherslazaroff.live closed. tsc clean; full suite 3300/0; eslint 0.
Last activity: 2026-06-09 — v11-04-02 applied + verified + shipped. This session: /paul:resume → /ui-ux-pro-max (BLOCKING) → branding.ts fields → layout generateMetadata + BL manifest → wordmark/vocab/perform-metadata → tsc/vitest/eslint → commit `f50060e387` + push → live deployed-surface probe (BL branded, CRC unchanged).
...
```
PLAN ──▶ APPLY ──▶ UNIFY        [v11-04-03 PLAN ✓ — next: APPLY (/ui-ux-pro-max BLOCKING)]
  ✓        ○        ○     (v11-04-01 + v11-04-02 SHIPPED+LIVE-VERIFIED; v11-04-03 plan created)
```
...
Last session: 2026-06-09 — `/paul:resume` → routed to v11-04-02 → /ui-ux-pro-max (BLOCKING) → applied all 3 tasks (branding.ts metadata fields · layout generateMetadata + BL manifest · wordmark/vocab/perform-metadata) → tsc clean / full suite 3300/0 / eslint 0 → committed `feat(v11-04-02)` `f50060e387` + pushed → live deployed-surface probe PASSED (BL /perform branded "Brothers Lazaroff", CRC byte-identical). HANDOFF-2026-06-09 archived.
Stopped at: v11-04-02 LOOP COMPLETE + SHIPPED + LIVE-VERIFIED. `feat(v11-04-02)` `f50060e387` pushed origin master, Vercel prod live (~04:38). Working tree clean (after this STATE bump).
Next action: **/paul:apply v11-04-03** (plan written; /ui-ux-pro-max already loaded this session → BLOCKING gate satisfied) — authed-dashboard read scoping. Key finding baked into the plan: getUpcoming/getRecent have NO live callers (only re-exported); the only live authed read path is `getSetlistsPage` (SSR /setlists + /api/setlists/page) + the 4 client subscribeToAllSetlists callers. Proxy sets `x-org-id` on ALL routes incl. /api (proxy.ts:150). v11-04-01 live probe already proved CRC setlists carry orgId (CRC /perform unchanged). **Authed deployed-probe needs a signed-in session → UAT-PENDING (no-local-dev).** Then phase v11-04 closes → **v11-05** = cross-tenant collection scoping (templates/roster/congregation/personnel R+W + CreationWizard vocab + in-app setlist-create orgId stamping); **v11-06** = isolation audit.
Resume file: none (v11-04-02 closed; HANDOFF-2026-06-09 archived to .paul/handoffs/archive/).
