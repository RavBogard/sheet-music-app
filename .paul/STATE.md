...
v50-07-03:     ✓ ──▶ ✓ ──▶ ○     [APPLY shipped — lazy hydration in SetlistGridHydrator + dual-read in useSetlistPerformance; +9 tests (1465/1465); tsc + next build clean; UNIFY pending]
v50-07-04..05: ○ ──▶ ○ ──▶ ○     [v50-07-04 Playwright kitchen-sink; v50-07-05 Sentry alarms + UAT prep]
```

## How to resume

Run `/paul:unify .paul/phases/v50-07-migration-cutover/v50-07-03-PLAN.md` to close the loop on v50-07-03 (lazy hydration + perf-view dual-read). UNIFY will: write SUMMARY (with diff vs PLAN, AC checklist, suite/build status), update ROADMAP.md, and stage the close commit. After UNIFY, plan v50-07-04 (Playwright kitchen-sink ≥100 runs random edits + airplane-mode + force-quits + cross-tab; reuse setupTwoWriterRace + SharedRemoteSubscriber + OfflineToggleAdapter from v50-06).
