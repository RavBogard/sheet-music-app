...
- `SetlistTrack` already has `type: 'song'|'header'|'reading'|'prayer'|'transition'|'note'`, plus `performer` ("Rabbi/Cantor/Congregation/Band"), `description` (body text for readings/prayers), `estimatedMinutes` — all in `src/types/models.ts`. The outline concept is present in the *data model*.
  - **CORRECTION (2026-08-30, post-Phase-3, verified):** this line originally claimed these fields are "all rendered by `src/components/performance/ServiceFlowCard.tsx`" and that the outline concept was "~70% present". **Both were wrong.** `ServiceFlowCard.tsx` is defined but imported by nothing (`grep -rn ServiceFlowCard src/` matches only its own definition) — it is dead code. The live Perform renderer is `src/components/performance/SetlistRow.tsx`, which shows `performer`/`notes`/`key` **only** inside its `isSong` branch and renders `description`/`estimatedMinutes` not at all. A non-song row displays only its title. The outline concept is ~0% present on the band's actual surface. This error did not affect Phases 1–3 (no UI work), but it inflated the premise for Phase 4 — which must therefore *build* outline rendering, not "polish" it.
