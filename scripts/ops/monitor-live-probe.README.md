...
`scripts/ops/monitor-live-probe.mjs` — the **autonomous live half** of the Monitor
...
node --env-file=.env.local scripts/ops/monitor-live-probe.mjs
...
node scripts/ops/monitor-live-probe.mjs
...
CRL_MCP_TOKEN=crl_live_<root> node scripts/ops/monitor-live-probe.mjs --mcp-only

# Validate plumbing with zero desk impact (mint → read → revoke, no writes):
CRL_MCP_TOKEN=crl_live_<root> node scripts/ops/monitor-live-probe.mjs --dry-run
```
