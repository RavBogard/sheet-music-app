#!/usr/bin/env bash
# scripts/dev/test-setup-coord-worktree.sh
#
...
#
# Usage:
#   bash scripts/dev/test-setup-coord-worktree.sh

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SCRIPTS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$SCRIPTS_DIR/.." && git rev-parse --show-toplevel)"

LIB="$SCRIPTS_DIR/lib/unshallow-current-repo.sh"
HELPER="$SCRIPTS_DIR/setup-coord-worktree.sh"
