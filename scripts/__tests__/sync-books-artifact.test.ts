...
describe("momentsDrift — a moved page is drift, a new timestamp is not", () => {
  const withMoments = (moments: unknown, builtAt: string, sha: string) =>
    JSON.stringify(
      { builtAt, sources: [{ book: "crc-kol-nidre", gitSha: sha }], moments },
      null,
      4,
    ) + "\n";
  ...
  const MOMENTS = [
    {
      id: "barchu",
      occurrences: [{ book: "crc-machzor-2008", unitId: "u1", folios: [100] }],
    },
  ];
    ...
    const moved = [
      {
        id: "barchu",
        occurrences: [
          { book: "crc-machzor-2008", unitId: "u1", folios: [101] },
        ],
      },
    ];
    expect(
      momentsDrift(
        withMoments(MOMENTS, "t", "sha"),
        withMoments(moved, "t", "sha"),
      ),
    ).toBe("DIFFERS");
  });
  ...
  it("no committed file yet is 'new', and unparseable content is DIFFERS rather than quietly equal", () => {
    ...
    expect(momentsDrift("{not json", withMoments(MOMENTS, "t", "sha"))).toBe(
      "DIFFERS",
    );
  });
});
...
describe("pickArtifact — newest live dist-app, and a refusal that says what to do", () => {
  ...
  it("ignores other artifacts in the same run", () => {
    const chosen = pickArtifact(
      listing([
        artifact({
          name: "shabbat-shacharit-80de868",
          archive_download_url: "wrong",
        }),
        artifact(),
      ]),
    );
...
describe("artifactToken — the refusal names the variable to set", () => {
  it("prefers the cross-repo PAT", () => {
    expect(
      artifactToken({
        SHIREISHABBAT_ARTIFACT_TOKEN: "pat",
        GITHUB_TOKEN: "ci",
      }),
    ).toBe("pat");
  });
  ...
  it("refuses with the name and the scope, not a bare 401 later", () => {
    expect(() => artifactToken({})).toThrow(
      /SHIREISHABBAT_ARTIFACT_TOKEN[\s\S]*actions:read/,
    );
  });
});
...
describe("resolveZipUrl — which endpoint each flag asks", () => {
  ...
  it("--from-run reads that run's artifacts", async () => {
    ...
    expect(seen).toBe(
      `${apiBase("RavBogard/ShireiShabbat")}/runs/35518946924/artifacts`,
    );
  });
...
describe("extractWanted — unzip, then tar, then a refusal naming both", () => {
  ...
  it("names both when neither exists, instead of failing as 'command not found'", () => {
    expect(() =>
      extractWanted("a.zip", "out", ((cmd: string) =>
        fail(`${cmd} ENOENT`)) as never),
    ).toThrow(/unzip[\s\S]*tar/);
  });
});

/**
 * The agreement check only runs if its two files come out of the artifact. A
 * well-meant trim of WANTED ("we don't read moments-pairs.json anywhere") would
 * silently turn `--strict` back into this repo agreeing with itself, and nothing
 * else in the suite would notice — the sync would still pass, just vacuously.
 * These lock the reason each name is on the list.
 */
describe("WANTED — the producer's own agreement check has to be unpacked to run", () => {
  it("asks for the checker and the pairs index it reads", () => {
    expect(WANTED).toContain("tools/moments_agree.py");
    expect(WANTED).toContain("moments-pairs.json");
  });

  it("still asks for the feeds, moments and books this repo consumes", () => {
    expect(WANTED).toContain("*-feed.json");
    expect(WANTED).toContain("moments.json");
    expect(WANTED).toContain("books.json");
  });

  it("asks for no PDF — the licensed bytes stay in the zip", () => {
    expect(WANTED.some((w) => w.includes(".pdf"))).toBe(false);
  });

  it("passes every wanted name through to unzip", () => {
    let seen: string[] = [];
    extractWanted("a.zip", "out", ((cmd: string, args: string[]) => {
      seen = args;
      return { status: 0 };
