...
```bash
node scripts/migrations/2026-05-24/backfill-drive-id-on-library-index.mjs > /tmp/backfill-dryrun.json 2> /tmp/backfill-dryrun.log
```
...
```bash
node scripts/migrations/2026-05-24/backfill-drive-id-on-library-index.mjs --apply > /tmp/backfill-apply.json 2> /tmp/backfill-apply.log
```
...
```bash
node scripts/migrations/2026-05-24/backfill-drive-id-on-library-index.mjs > /tmp/backfill-verify.json 2> /tmp/backfill-verify.log
```
...
- `scripts/migrations/2026-05-23/rebuild-setlist-fileids-denorm.mjs` (precedent shape)
