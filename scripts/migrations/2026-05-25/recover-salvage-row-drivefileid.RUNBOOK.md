...
- Non-salvage rows (handled by W4-2 sibling at
  `scripts/migrations/2026-05-24/backfill-drive-id-on-library-index.mjs`).
...
```bash
node scripts/migrations/2026-05-25/recover-salvage-row-drivefileid.mjs > .paul/ops/salvage-row-recovery/DRY-RUN-001.log 2>&1
```
...
```bash
node scripts/migrations/2026-05-25/recover-salvage-row-drivefileid.mjs --apply > .paul/ops/salvage-row-recovery/APPLY-001.log 2>&1
```
...
```bash
node scripts/migrations/2026-05-25/recover-salvage-row-drivefileid.mjs > .paul/ops/salvage-row-recovery/REDRY-001.log 2>&1
```
...
- `.paul/ops/backfill-drive-id/RUN-SUMMARY.md` (W4-2 narrow Option-A
  ship summary)
- `scripts/migrations/2026-05-24/backfill-drive-id-on-library-index.mjs` (W4-2 sibling)
