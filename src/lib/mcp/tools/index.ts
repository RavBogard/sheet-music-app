...
export { registerSetlistReaderBearerTools } from "./setlist-reader-bearer"
...
export function registerWriteTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        {
            description:
                "Propose a liturgyRef (unit id + printed page) for every unbound row of a setlist or template, by matching the row's TITLE against the liturgy lookup for `book`. Bind, don't add: no row is created, removed or reordered — the row count is unchanged. Returns `{bound, plausible, unmatched, identified, skipped}`. `bound` is safe to write (an exact or near spelling, unambiguous); `plausible` is real candidates that need Daniel — show him the alternatives and pass his picks back in `accept`. Headers are NEVER bound (a header is a sign, not a moment), and a row that already carries a liturgyRef is never overwritten — but a setlist row that already has a page in `book` and no unitId is returned in `identified` when its title matches clearly ON THAT SAME PAGE; a real run adds only `liturgyRef.unitId` + `momentId` to those rows and never changes the page. For crc-friday / crc-saturday the unit ids are the legacy feed's (`…@legacy-shabbat-evening` / `…@legacy-shabbat-morning`), which print the booklet's own page numbers. `dryRun` defaults TRUE and writes nothing. A real run (`dryRun:false`) writes `bound` plus only the `accept` rows, and only where the page validates against the registry — an identity-only match with no page in this book is reported, never written with a borrowed number. Lookup tables exist for crc-friday, crc-saturday, shabbat-maariv and shabbat-shacharit; any other book is refused rather than guessed at. Admin + band_leader only.",
            inputSchema: {
                setlistId: z
                    .string()
                    .min(1)
    ...
    server.registerTool(
        ...
        {
            description:
                "Probe a single chart's health (Storage + Drive fallback) without downloading bytes. Returns { ok: true, fileId, health } where `health` is one of: { status: 'ok', source: 'firebase-storage'|'google-drive', mimeType? }, { status: 'missing', reason }, or { status: 'unreachable', error }. E4 (R-0904-live-cw-3, 2026-09-04): `ok` now requires reachable bytes AND a `library_index` row. A fileId whose bytes sit at a candidate path with NO index row returns `{ status: 'bytes_without_index_row', reason }` instead of a green — that shape is not openable, because `download_chart` keys on `library_index` and answers `chart_not_found`, while `search_library` reads `songs` and may still show the row. A Firestore read failure fails SOFT: an unknown catalog is not evidence of absence, so the probed health stands. Use to verify a bond is renderable before bonding it onto a setlist row, or to investigate why a published chart isn't loading for the band. Cheap — metadata-only, no byte transfer.",
            inputSchema: {
                fileId: z
                    .string()
                    .min(1)
    ...
    server.registerTool(
        ...
        {
            description:
                "Admin-only one-shot idempotent library_index hygiene sweep — finds active rows whose normalized names collide (e.g. `\" Ana B_Koach.pdf\"` leading-space dupes; or two uploads of the same chart name) and marks all-but-one `status: \"duplicate\"`. Canonical pick, in order: `active` outranks every other status (L1-W2 — a hidden row taken as canonical empties the group, since the losers are hidden by the mark this run writes), then a real-bytes row outranks a Google-Doc, then earliest `uploadedAt`, then fileId asc. Losers also get the status mirrored into `songs/{id}` when that doc exists. searchLibrary + list_library hide `status: \"duplicate\"` from their default surface, so the practical effect is collapsing dupes to one visible row without deleting any bytes. Already-`duplicate` rows are skipped → safe to re-run. L1-W3 (2026-09-01): TENANT-SCOPED — scans only the caller's org (`rowOrg(row.orgId) === callerOrg`, the same filter list_library applies), so `coverage.total` now agrees with the browse. Before this it scanned the raw collection and could plan or commit writes against another tenant's rows. L1-W2 (2026-09-01): `archived` rows are NO LONGER skipped — they are in the hygiene scan now, and list_library's default browse hides them instead, so the two sides finally agree. CYCLE-3 MCP-001 (Daniel-ratified 2026-05-18T18:45Z): F-05 contract aligned with reconcile/backfill — dryRun-default; a real run without `force: true` returns the plan with `refused: true` and no writes. Optional `forceScore` (0..1) enables an ADDITIONAL fuzzy-similarity grouping pass on top of the default exact-normalize: Levenshtein-similarity > threshold clusters rows that survived exact-grouping. PER-CALL TUNING ONLY — the standing 0.85 strict threshold elsewhere in the codebase is unchanged. Omitting `forceScore` preserves the historical exact-normalize-only behavior. Returns `{scanned, groupsFound, wouldMark, committed, songsMirrored, groups[], dryRun, refused?, threshold, coverage:{total,eligible,scanned,filteredOut}}` — `wouldMark` is the planned loser count (surfaced on every path); `committed` is the count actually marked this call (0 on dryRun/refused, equals `wouldMark` on a committed real-run). W2 (R-0903-live-cw-2 §5) — REVERSIBILITY PRECEDES HIDING: every real run writes a `dedupeRuns/{runId}` record holding each marked row's `priorStatus` BEFORE any status is written, and returns that id as `dedupeRunId`; hand it to `undo_dedupe_group` to reverse the run. A run that marks nothing writes no record. W5 (§3, §4, §6) — the response gained three fields. `hashGroups` is the BYTE-IDENTITY lane: rows grouped by `contentHash` sha256 rather than by name, so it sees pairs no name pass could (`gminor_spirits` / `G-minor Spirits` share no normalized name and are the same 42,729 bytes). It is REPORT ONLY and marks nothing — every new mark decided by bytes is Daniel's, per cluster — it scans EVERY status including `duplicate` (a cluster with one visible row is one whose mark is byte-justified and needs no decision), it reports CLUSTERS rather than pairs (three rows is common), and `noActionReason` says when a group is a conclusion rather than an action. `hashPassCoverage:{hashed,unhashed,hashFailed}` distinguishes \"no byte pairs\" from \"nothing hashed yet\" — run `backfill_content_hash` first or this lane reports nothing meaningful. `filterOrder` states the order the filters actually ran in, which is why `list_library` can report a different marked count: its non_chart filter runs before its status filter, so a row caught by the earlier is never counted by the later. Every group row now carries the DECIDING fields — `mimeType`, `sizeBytes`, `status`, `contentHash`, `bondCount` — not just name and date. Canonical pick gained BONDEDNESS between the Google-Apps rule and `uploadedAt`, counted only from LIVE in-tenant setlists, so use stops losing to age. E1 (R-0904-live-cw-3, 2026-09-04) — THE FORMAT-CLASS GATE, applied by every lane on group EMISSION: a group whose rows span more than one class (`score` | `audio` | `text` | `gapps` | `unknown`) is NOT EMITTED by the exact, fuzzy or hash lane, and an all-`unknown` group is refused outright. This closes the hole `forceScore` opened: audio previously classed as `score`, so the fuzzy lane reassembled the very group the exact lane's key was fixed to prevent — measured on the live catalog at 0.85, TWO groups pairing a PDF chart with its own mp3 recording, one of them marking the canonical row Daniel had personally chosen. The response gained `formatClassRefusals`: how many groups the gate refused, because a gate that cannot say how often it fired cannot be trusted to have fired. `0` means no near-name spanned two classes on this scan, NOT that the gate is off. The canonical picker's Google-Apps demotion is RETIRED in the same change — the gate refuses that group earlier and more completely, so neither row is marked rather than the Google-Doc losing.",
            inputSchema: {
                dryRun: z
                    .boolean()
                    .optional()
        ...
        async (args, extra) =>
            jsonResult(
                await dedupeLibraryIndex(uidFrom(extra), args, orgFrom(extra)),
            ),
    )
    ...
    server.registerTool(
        ...
        {
            description:
                "Admin-only REVERSAL of a `dedupe_library` run — the missing half of a destructive operation. Until this tool existed, dedupe could hide rows and nothing could un-hide them: 103 rows in production carry `status: \"duplicate\"` and the only reversal artifact was a hand-written JSON file in another repository. TWO MODES, and no third. (1) `runId` — restore every row that run marked to its RECORDED `priorStatus`, read from `dedupeRuns/{runId}`; an `archived` row goes back to `archived`, never to `active`. (2) `fileId` + `toStatus` — restore one row to a status YOU name. A call with neither is REFUSED: a blanket restore would have to invent a prior status, the only invention available is `active`, and 18 of the rows with a recorded prior status were `archived` before dedupe ran — so the refusal names that count rather than un-archiving rows somebody deliberately archived (R-0903-live-cw-2 §5: a repair tool may not create the class of harm it exists to repair). Restoring clears `dedupedAt` / `dedupeRunId` / `priorStatus` and mirrors the status onto `songs/{id}` when that doc exists. Tenant-scoped: a row belonging to another org is skipped, never written. Idempotent and honest about it — a row that is no longer `duplicate` is reported as skipped WITH the reason, not silently written over. F-05: `dryRun` reports the full plan without writing and never needs `force`; a real run requires `force: true`. Returns `{mode, runId?, recordedRows?, restored, skipped, dryRun, refused?, rows[]}` where each row carries `{fileId, fromStatus, toStatus, source, songMirrored, skipped?}` — `source` says whether the target status came from the run record or from your explicit argument.",
            inputSchema: {
                runId: z
                    .string()
                    .optional()
                    .describe(
                        "Restore every row of this dedupe run to its RECORDED priorStatus (from `dedupeRuns/{runId}`). The runId is returned as `dedupeRunId` by `dedupe_library` on any run that marked rows. Mutually exclusive with `fileId`.",
                    ),
                fileId: z
                    .string()
                    .optional()
                    .describe(
                        "Restore this one library_index row. REQUIRES `toStatus` — the tool will not infer a target, because inferring is how an archived row becomes active. Mutually exclusive with `runId`.",
                    ),
                toStatus: z
                    .string()
                    .optional()
                    .describe(
                        "The status to restore `fileId` to, e.g. \"active\" or \"archived\". Required with `fileId`. Ignored in runId mode, where each row's recorded priorStatus decides.",
                    ),
                dryRun: z
                    .boolean()
                    .optional()
                    .describe(
                        "When true, returns the full restore plan — every row, its current status, and where its target status came from — without writing. F-05 standing rule: dryRun does NOT require force.",
                    ),
                force: z
                    .boolean()
                    .optional()
    ...
    server.registerTool(
        ...
        {
            description:
                "Admin-only, batched and RESUMABLE — populate `library_index.contentHash` by reading each row's bytes once. The persisted shape is `{alg:'sha256', value, sizeBytes, at, source}`. This is the only library tool that costs real time and bandwidth: it downloads every chart. It is safe to interrupt and safe to re-run — a row whose recorded `contentHash.sizeBytes` still matches its `fileSize` is skipped WITHOUT a byte read, so a re-run after an interruption resumes and a re-run after a completed pass is free. Use `limit` to take a bite at a time. WHY SHA256 AND NOT THE FREE METADATA MD5 (R-0903-live-cw-2 §3): the md5 is not uniform — Google-Apps rows carry no `md5Checksum` at all — so a column holding whichever hash was cheapest would silently split a true pair whose two rows were hashed by different routes, and a confident non-match is worse than an absence. THE MD5 IS STILL USED, as a CROSS-CHECK: where a row claims a `driveMd5` (hex) or a Storage `md5Hash` (base64), the md5 recomputed from the downloaded bytes must agree. A mismatch means we did not fetch the bytes this row claims, so the row is recorded `hashFailed` with the mismatching values and NO `contentHash` is written — a wrong hash makes a false pair confidently. Rows whose bytes are unreachable (Google-Apps rows, dead-byte rows) are returned as a named POPULATION in `failures`, never skipped in silence. Every status is in scope, `duplicate` included: a marked row's bytes are exactly what decides whether the mark was right — and so is every `non_chart` row, the audio part-tracks included (R-0903-live-cw-3 §3). TWO POPULATIONS, REPORTED SEPARATELY: `populations.chart` and `populations.nonChart` each carry `{rows, read, bytesRead, hashed, alreadyCurrent, failed}`, partitioned by the same `isNonChartArtifactShape` classifier the browse surfaces use — note that it counts Google-Apps rows as nonChart too. They are never blended, because there are two different questions waiting on them and a single number answers neither. `bytesRead` is in BYTES, not rows, because the audio population runs to roughly 1.5 MB per group against a chart population of a few hundred KB in total: the honest answer to that cost is to let the run go long, not to sample it. Returns `{scanned, read, hashed, alreadyCurrent, failed, remaining, populations:{chart,nonChart}, md5CrossCheck:{claimed,agreed,mismatched}, failures[], byteIdenticalClusters[], dryRun, coverage}` — `byteIdenticalClusters` is the answer the wave exists for, and it reports clusters (which are often 3 rows, not 2), not just pairs. STOP CONDITION: if `md5CrossCheck.mismatched` is not near zero, the download path and the rows disagree about which object belongs to which row — that is a finding to report, not a retry. F-05: `dryRun` reports everything without writing and never needs `force`.",
            inputSchema: {
                dryRun: z
                    .boolean()
                    .optional()
    ...
    server.registerTool(
        ...
        async (args, extra) =>
            jsonResult(
                await backfillLibraryIndex(uidFrom(extra), args, orgFrom(extra)),
            ),
    )
    ...
    server.registerTool(
        ...
        {
            description:
                "Admin-only one-shot bootstrap reconciliation for `library_index` rows under the storage-canonical direction (cycle-3 ADDENDUM-1 NEW-2). Walks every active row; for any whose Storage object 404s, probes Drive: Drive 200 + chart-shape → mirror the bytes into Storage at the EXISTING fileId (preserving every setlist/song bond) and flip `status: 'active'`; Drive 200 + non-chart mime (folder / audio / .DS_Store / Office doc) → route to `skippedNonChart` bucket and leave untouched (cycle-3 BUG-001 — prevents force-writes of 0-byte garbage at the existing fileId); Drive 404 → mark `status: 'orphaned'`; canonical mime is a Drive shortcut → route to the `needsRebond` bucket (C9I3-002), and on a force-run auto-resolve the shortcut TARGET's bytes in place at the same fileId (preserving bonds) — a target that 404s escalates to `orphan`, a shortcut chain / unresolvable target stays `needsRebond` for a manual re-bond; Drive 5xx / timeout → leave the row untouched and surface in the `transient` bucket so the operator can re-run later. Drains the ~250 dead-looking rows from the pre-NEW-1 era. Idempotent: rows already `status: 'orphaned'` or `status: 'duplicate'` are skipped, so a second run after a successful force-run leaves nothing to do. Defaults `dryRun: true` per the F-05 dry-run-is-observability rule — caller MUST pass `force: true` to actually write. dryRun returns the full plan (bucket counts + per-row preview, capped at 500 rows per bucket) without writes. Real run without `force: true` returns the rich `force_required` envelope (REG-003: `{ok:false, error:{machine_code:'force_required'}, dryRunPlan:<the plan>}`) and no writes. Mirror operation preserves processChartUpload's atomic-guard contract (read-verify + compensating-delete + library_signals broadcast). Returns `{scanned, alreadyHealthy, driveMirror:{count,rows,truncated}, orphan:{count,rows,truncated}, transient:{count,rows,truncated}, skippedNonChart:{count,rows,truncated}, needsRebond:{count,rows,truncated}, coverage, dryRun, committed}` on success. L1-W3 (2026-09-01): TENANT-SCOPED — scans only the caller's org (`rowOrg(row.orgId) === callerOrg`, the same filter list_library applies), so `coverage.total` now agrees with the browse. Before this it scanned the raw collection and could probe and re-status another tenant's charts.",
            inputSchema: {
                dryRun: z
                    .boolean()
                    .optional()
        ...
        async (args, extra) =>
            jsonResult(
                await reconcileLibrary(uidFrom(extra), args, orgFrom(extra)),
            ),
    )
...
export function registerMonitorTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        {
            description:
                "Probe the X32 monitor bridge's health remotely (admin / band_leader). One call returns a DERIVED `alive` verdict computed from how long ago the bridge last checked in — because the raw `status`/`x32Connected` fields are last-write-wins with NO expiry and keep reading \"online\"/true for hours after the bridge has actually died. Use this (not the raw fields) to know whether the bridge is up. Returns `{alive, lastSeenAgeS, stateAgeS, stateStale, leaseExpired, standby, status, x32Connected, socketAlive, unconfirmedCount, queueDepth, version, clients, uptimeMs, errCount, lastError, summary}`. `alive` = last heartbeat within ~2 minutes; `standby` (bridge v10.0.8+) is non-null when a bridge process is RUNNING at the venue but does not hold the single-writer lease — `alive:false` WITH a fresh `standby` means a bridge is there and mid-takeover (wait ~30s and re-probe), while `alive:false` with `standby:null` means nothing is running at all and someone has to start it; `stateStale` = the live mixer-values snapshot is older than ~90s (values not current); `socketAlive`/`unconfirmedCount`/`queueDepth`/`uptimeMs`/`errCount`/`lastError` are the v10.0.4 diagnostics (null against an older bridge) that split socket-dead from state-wedged and surface the most recent error. `summary` is a one-line human verdict.",
            inputSchema: {},
        },
        async (_args, extra) =>
