...
export function registerBatchIntakeTools(server: McpServer): void {
    ...
    registerAppTool(
        ...
        "open_chart_dropzone",
        {
            ...
            description:
                "Open an interactive drop zone so the user can add many chart files to the library at once. YOU NEVER HANDLE THE FILE BYTES — this renders a small upload panel next to the conversation, the user drops or picks files there, and the panel uploads them straight to storage and commits the batch on its own. FALLBACK ONLY: prefer get_chart_inbox + sync_chart_inbox (the shared Drive inbox folder), which works in every client and on phones; use this drop zone only when the user explicitly asks for an in-chat upload panel, and know that some clients cannot render it. It returns a batchId: after the user says they are done, call get_upload_batch with it to see what landed, then talk them through anything that came back 'parked' as a possible duplicate and act on their answer with resolve_upload_item. For a Google Drive folder use import_drive_folder instead. From Claude Code on Windows the equivalent is the CLI courier `node scripts/upload-batch.mjs <files...>`.",
            inputSchema: {
