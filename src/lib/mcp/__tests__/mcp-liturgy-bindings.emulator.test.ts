...
describe("MCP propose_liturgy_bindings (emulator)", () => {
    ...
    it("a real run writes the bound rows and NOT the plausible one", async () => {
        ...
        // Page only. The unit id belongs to shabbat-maariv, and a pagemap book
        // has no units — the registry refuses it and the write drops it.
        expect(after.t3).toEqual({ book: "crc-friday", folio: 10 })
    ...
    it("writes a plausible row only when Daniel named it", async () => {
        ...
        expect((await refs()).t5).toEqual({ book: "crc-friday", folio: 30 })
    })
