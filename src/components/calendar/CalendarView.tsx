        ...
        <div className="flex flex-col h-full bg-card rounded-xl overflow-hidden border border-border">
            ...
            <div className="grid grid-cols-7 border-b border-border bg-muted">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, i) => (
                    <div key={day} className={`p-3 text-sm font-medium text-center ${i === 6 || i === 5 ? 'text-blue-400' : 'text-muted-foreground'}`}>
                        {day}
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-card overflow-y-auto">
                {days.map((date, i) => {
                    ...
                    return (
                        <div key={date.toISOString()} className={`relative p-2 border-b border-r border-border group transition-colors hover:bg-muted/20 ${isToday ? 'bg-blue-500/5' : ''}`}>
                            <div className="flex items-center justify-between mb-2">
                                <span className={`text-sm font-medium h-7 w-7 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-600 text-foreground' : 'text-muted-foreground'}`}>
                                    {date.getDate()}
                                </span>
                                {(isSaturday || isFriday) && (
                                    <Button
                                        size="icon"
                                        variant="ghost"
                                        className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-blue-400"
                            ...
                            <div className="space-y-1">
                                ...
                                {daySetlists.length === 0 && isSaturday && (
                                    <button
                                        ...
                                        className="w-full text-left p-1.5 rounded-md text-xs border border-dashed border-border text-muted-foreground/60 hover:text-blue-400 hover:border-blue-400/50 transition-colors flex items-center gap-1"
                                    >
                                        <Plus className="h-3 w-3" />
                                        Shabbat
                                    </button>
